...
class Hashing
{
    ...
    int collision_function_quadratic_probing(int);
...
int Hashing::collision_function_quadratic_probing(int data)
{
 int counter=1;
 while(true)
 {
  if(Hashing_table[(hashing_function(data)+counter*counter)%7]==-1)
  {
      return (hashing_function(data)+counter*counter)%7;
  }
   counter++;
 }
}
void Hashing::add_value_in_hash_table(int input)
{
    ...
    {
    check=collision_function_quadratic_probing(input);
    {
        Hashing_table[check]=input;
    }
    }
...
int Hashing::search_in_hash_table(int input)
{
    ...
    else
  {
    ...
    while(true)
    {
    if(Hashing_table[(hashing_function(input)+counter*counter)%7]==input)
    {
    return 1;
    }
    counter++;
...
int Hashing::delete_in_hash_table(int input)
{
    ...
    else
  {
    ...
    if(Hashing_table[(hashing_function(input)+counter*counter)%7]==input)
    {
        Hashing_table[(hashing_function(input)+counter*counter)%7]=-1;
        return 1;
    }
    counter++;
    if(counter>7)
